...
def her(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
    ...
    env, test_env = env_fn(), env_fn()
    obs_dim = env.get_observation().shape[0] + env.get_goal().shape[0]
    act_dim = env.action_dim
    ...
    for t in range(total_steps):
        ...
        # use the learned policy (with some noise, via act_noise). 
        if t > start_steps:
            a = get_action(og, act_noise)
        else:
            a = random_action(env)

        # Step the env
        o2, r, d, _ = env.step(a)
        og2 = np.concatenate((o2, g))
        r = env.get_reward(o2, g)
        ep_ret += r
        ep_len += 1
        ...
        # Store experience to replay buffer
        replay_buffer.store(og, a, r, og2, d)
        ...
        # End of trajectory handling
        if d or (ep_len == max_ep_len):
            # HER: generate new virtual goals and store it in replay buffer
            # final strategy: last state of episode becomes the goal
            virtual_goal = env.as_goal(o)
            og_virtual = np.concatenate((o, virtual_goal))
            
            # TODO: wrong. instead, resave the entire trajectory with the new virtual goal
            replay_buffer.store(og_virtual, a, env.get_reward(o, virtual_goal), og2, d)
...
if __name__ == '__main__':
    ...
    def make_env():
        ...
        def as_goal(obs=None):
            if obs is None:
                obs = env._get_observations()
            return obs['object-state'][:3] # object position
        env.get_goal = as_goal
        ...
        def get_reward(obs, goal):
            ...
            return 1 if np.linalg.norm(obs['object-state'][:3] - goal) < 0.05 else 0
